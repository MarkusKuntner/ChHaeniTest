    ...
    partial class Zeitschleife
    {
        ...
        public static (float, float) IntStandCalculate(int nteil, float Roughness, float DiffBuilding, float windge, float sigmauHurley)
        {
            ...
            if ((Program.IStatistics == Consts.MeteoZR) || (Program.IStatistics == Consts.MeteoSonic)) 
            {
                ...
                {
                    U0int = Program.U0[1] * windge / Program.U0initial[1];
                    V0int = Program.V0[1] * windge / Program.U0initial[1];
                }
                else if (DiffBuilding >= Program.MeasurementHeight[Program.MetProfileNumb])
                {
                    U0int = Program.U0[Program.MetProfileNumb] * windge / Program.U0initial[Program.MetProfileNumb];
                    V0int = Program.V0[Program.MetProfileNumb] * windge / Program.U0initial[Program.MetProfileNumb];
                }
                else
                {
                    int ipo = 0;
                    ...
                    U0int = Program.U0[ipo - 1] + (Program.U0[ipo] - Program.U0[ipo - 1]) * help * windge / Program.U0initial[ipo];
                    V0int = Program.V0[ipo - 1] + (Program.V0[ipo] - Program.V0[ipo - 1]) * help * windge / Program.U0initial[ipo];
                }
            }
